function letters = getDiffAlphabet(width, height, num)
if num == 1
    %only numbers
    chars = [char(48:57)];
elseif num == 2
...
for i=1:length(chars)
    ...
    data = measure(image,label(image),{'Perimeter','Size'});
    [dataPer,index] = max(data.perimeter);
    dataPer = data.size(index);
    letters(i) = abs(dataPer/dataPer);
end    
end